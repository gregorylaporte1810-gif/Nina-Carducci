...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const tags = [...new Set(items.map((item) => item.dataset.galleryTag).filter(Boolean))];
  ...
  items.forEach((item) => {
    ...
    column.className = 'item-column mb-4 col-12 col-sm-6 col-md-4 col-lg-4 col-xl-4';
  ...
  tags.forEach((tag) => tagsBar.appendChild(createTagButton(tag, tag)));
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-body position-relative">
          <button type="button" class="mg-prev" aria-label="Image précédente" style="cursor:pointer;position:absolute;top:50%;left:-15px;background:white;border:0;z-index:1;">&lt;</button>
          <img class="lightboxImage img-fluid" alt="">
          <button type="button" class="mg-next" aria-label="Image suivante" style="cursor:pointer;position:absolute;top:50%;right:-15px;background:white;border:0;z-index:1;">&gt;</button>
        </div>
      </div>
    </div>`;
  ...
  const modal = typeof bootstrap !== 'undefined' ? new bootstrap.Modal(lightbox) : null;
  let activeImage = null;

  const getActiveTag = () =>
    gallery.querySelector('.tags-bar .active-tag')?.dataset.imagesToggle ?? 'all';

  const getFilteredItems = () => {
    const activeTag = getActiveTag();
    return items.filter(
      (item) => activeTag === 'all' || item.dataset.galleryTag === activeTag
    );
  };
  ...
  const displayInLightbox = (item) => {
    ...
    lightboxImage.alt = item.alt || 'Photographie de la galerie';
  };
  ...
  const navigate = (direction) => {
    ...
    const nextIndex = (index + direction + visibleItems.length) % visibleItems.length;
  ...
  tagsBar.addEventListener('click', (event) => {
    const button = event.target.closest('[data-images-toggle]');

    if (!button) {
      return;
    }

    tagsBar.querySelectorAll('[data-images-toggle]').forEach((tagButton) => {
      const isActive = tagButton === button;
      tagButton.classList.toggle('active', isActive);
      tagButton.classList.toggle('active-tag', isActive);
    ...
    items.forEach((item) => {
      ...
      const shouldShow = selectedTag === 'all' || item.dataset.galleryTag === selectedTag;
